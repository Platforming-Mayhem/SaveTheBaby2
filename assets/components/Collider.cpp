...
namespace K 
{
	...
	void Collider::CapsuleColliderStatic()
	{
		if (K::Physics::CanGetClosestPoint())
		{
			...
			{
				*(this->parent->GetTransform()->position) += K::Vector3(0.0f, 0.0f, -this->time * K::Time::deltaTime() * 48.0f);
		...
		*temp.position = this->GetPosition();
		...
		if (K::Physics::CanGetClosestPoint())
		{
			...
			for (K::ContactPoint pointOnLine : K::Physics::GetClosestPoints(this->GetPosition()))
			{
				...
				{
					glUniform3f(this->parent->GetMaterial()->GetShader()->GetUniform("colorTint"), 1.0f, 0.0f, 0.0f);
				}
				else 
				{
					glUniform3f(this->parent->GetMaterial()->GetShader()->GetUniform("colorTint"), 0.0f, 0.0f, 1.0f);
				}
