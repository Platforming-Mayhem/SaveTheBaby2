...
namespace K 
{
	...
	void ButtonLoadScene::Update() 
	{
		...
		{
			float r = std::lerp(this->mesh->GetColourTint().rgb[0], this->unSelectedTint[0], K::Time::deltaTime() * 60.0f);
			float g = std::lerp(this->mesh->GetColourTint().rgb[1], this->unSelectedTint[1], K::Time::deltaTime() * 60.0f);
			float b = std::lerp(this->mesh->GetColourTint().rgb[2], this->unSelectedTint[2], K::Time::deltaTime() * 60.0f);
			this->mesh->SetColourTint(r, g, b);
		}
	}
	...
	void ButtonLoadScene::SetPropertyValues(const char* value, int valueIndex)
	{
		if (value[0] != '\0' && value != nullptr)
		{
			...
			case 2:
				...
				break;
			case 3:
				this->unSelectedTint[0] = std::stof(temp);
				break;
			case 4:
				this->unSelectedTint[1] = std::stof(temp);
				break;
			case 5:
				this->unSelectedTint[2] = std::stof(temp);
				break;
			}
		}
	}
	...
	const char* ButtonLoadScene::GetPropertyValues()
	{
		...
		this->properties += std::to_string(this->unSelectedTint[0]) + ",";
		this->properties += std::to_string(this->unSelectedTint[1]) + ",";
		this->properties += std::to_string(this->unSelectedTint[2]);
